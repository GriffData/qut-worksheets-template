...
(c) Apply your rule to each element in the `test_set` vector below, to decide which (if any) are outliers.
(d) Does your answer change if you use the sample mean and sample variance, rather than the given mean and variance values? Why do you think that is?^[Hint: `do.call("rbind",test_set)` can convert the data to a `matrix` then use `var` and `colMeans` to calculate the sample information.]
